...
void tests::z80_tests::load(const uint16_t offset, const string& filename) {
...
bool tests::z80_tests::runTestCycles() {
...
void tests::z80_tests::out(const uint8_t p, const uint8_t v) {
	if (p == 0) {
		finished = true;
		return;
	}
...
bool tests::z80_tests::runTest(const string& filename, const uint64_t cycles_expected) {
	uint64_t cycles = 0;
	load(0x100, filename);
	memory[0] = 0xD3;
	memory[1] = 0x00;
